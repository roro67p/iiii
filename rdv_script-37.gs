...
function pageEditCoiff(e) {
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    +'.hdr{background:linear-gradient(135deg,#6366F1,#4F46E5);padding:16px;display:flex;align-items:center;gap:12px;margin-bottom:0}'
    ...
    +'<div class="r2"><div><label>Debut</label><input type="time" name="debut" value="'+d.debut+'"></div>'
    +'<div><label>Fin</label><input type="time" name="fin" value="'+d.fin+'"></div></div></div>'
    ...
    +'<input type="text" name="conges" placeholder="2026-04-15, 2026-04-20"></div>'
    +'<div class="sec"><div class="st">Vacances</div>'
    +'<div class="r2"><div><label>Du</label><input type="date" name="vdebut"></div>'
    +'<div><label>Au</label><input type="date" name="vfin"></div></div></div>'
    +'<div class="sec"><div class="st">Prime EU</div>'
    +'<input type="number" name="prime" value="'+d.prime+'" placeholder="0"></div>'
    ...
    +'<script>var offs=[];function tj(el){var j=el.getAttribute("data-j");if(el.classList.contains("on")){el.classList.remove("on");offs=offs.filter(function(x){return x!==j;});}else{el.classList.add("on");offs.push(j);}document.getElementById("joff").value=offs.join(",");}</script>'
    +'</body></html>';
...
function pageSaveCoiff(e) {
  var url=ScriptApp.getService().getUrl();
  if(e.parameter.pwd!==CONFIG.MOT_DE_PASSE) return err("Erreur mot de passe",url,e.parameter.pwd);
  var conges="";
  if(e.parameter.joff) conges+="Repos:"+e.parameter.joff+" ";
  if(e.parameter.conges) conges+="Conges:"+e.parameter.conges+" ";
  if(e.parameter.vdebut&&e.parameter.vfin) conges+="Vacances:"+e.parameter.vdebut+"-"+e.parameter.vfin;
  var ws=getSheetCoiff();
  var vals=ws.getDataRange().getValues();
  var found=false;
  for(var i=1;i<vals.length;i++){
    if(String(vals[i][0])===e.parameter.nom){
      ws.getRange(i+1,2).setValue(e.parameter.debut||"09:00");
      ws.getRange(i+1,3).setValue(e.parameter.fin||"19:00");
      ws.getRange(i+1,4).setValue(conges.trim());
      ws.getRange(i+1,5).setValue(parseFloat(e.parameter.prime)||0);
      found=true; break;
    }
  }
  if(!found) ws.appendRow([e.parameter.nom,e.parameter.debut||"09:00",e.parameter.fin||"19:00",conges.trim(),parseFloat(e.parameter.prime)||0]);
  return redirect(url+"?page=admin&pwd="+e.parameter.pwd,"Sauvegarde OK !");
}
